package br.edu.ifsp.arq.dw2s6.projeto_final.exceptionhandler;
...
import br.edu.ifsp.arq.dw2s6.projeto_final.service.exception.PessoaInexistenteOuInativaException;
import br.edu.ifsp.arq.dw2s6.projeto_final.service.exception.SenhaDiferenteException;

@ControllerAdvice
public class OrdemServicoExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<Object> handleSenhaDiferenteException(SenhaDiferenteException ex) {
		String mensagemUsuario = messageSource.getMessage("senha.incompativel", null, LocaleContextHolder.getLocale());
		String mensagemDesenvolvedor = ex.toString();
		List<Erro> erros = Arrays.asList(new Erro(mensagemUsuario, mensagemDesenvolvedor));
		return ResponseEntity.badRequest().body(erros);
	}
